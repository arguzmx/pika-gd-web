...
export class ConstructorMenu {
  ...
  public CreaMenu(menu: MenuAplicacion, acl: ACLUsuario): NbMenuItem[] {
    if (menu && acl) {
      ...
      menu.Elementos.forEach((el) => {
        if (environment.clouded && el.Icono == "settings-2-outline") {
          let idx = -1;
          for (let i = 0; i < el.Hijos.length; i++) {
            const encontrado = el.Hijos[i].Parametros.findIndex(
              (h) => h.Valor == "TareaAutomatica"
            );
  ...
  private CreaItem(el: ElementoMenu): NbMenuItem {
    ...
    if (this.acl.EsAdmin) {
      ...
      if (el.TokenMod && el.TokenApp) {
        const permiso = this.acl.Permisos.find(
          (x) => x.AplicacionId === el.TokenApp && x.ModuloId === el.TokenMod
        );
        if (permiso) {
  ...
  private TieneLinksActivos(item: NbMenuItem): boolean {
    ...
    if (item.children) {
      let activos = false;
      const cuenta = item.children.filter(
        (x) => x.link != "" && x.group == false
      ).length;
      if (cuenta > 0) {
        return true;
      } else {
