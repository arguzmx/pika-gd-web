...
import {
  ...
  PEJECUTAR,
  PermisoACL,
} from "./../../@pika/seguridad/permiso-acl";
import {
  Propiedad,
  IProveedorReporte,
  PermisoAplicacion,
...
export class EntidadesService {
  ...
  private BusFiltros = new BehaviorSubject<EventosFiltrado>(
    EventosFiltrado.Ninguno
  );

  constructor(
    private app: AppConfig,
    private sesion: SesionQuery,
    private cache: CacheEntidadesService,
  ...
  ObtienePermiso(appid: string, moduloId: string): PermisoAplicacion {
    ...
    if (this.sesion.ACL.EsAdmin) {
      ...
      const acl = this.sesion.ACL.Permisos.find(
        (x) => x.ModuloId === moduloId && x.AplicacionId === appid
      );
      ...
      if (acl) {
        ...
        if (
          environment.clouded &&
          (moduloId == "ADMIN-CONFIGURACION-VOLUMEN" ||
            moduloId == "ADMIN-CONFIGURACION")
        ) {
          permiso.Admin = false;
          permiso.Leer = true;
          permiso.Eliminar = false;
          permiso.Escribir = false;
          permiso.Ejecutar = false;
          permiso.Mascara = PLEER;
        }
      }
  ...
  CreaPermiso(
    ...
    if (this.sesion.ACL.EsAdmin) {
      ...
      const acl = this.sesion.ACL.Permisos.findIndex(
        (x) => x.ModuloId === moduloId && x.AplicacionId === appid
      );
  ...
  public ObtienePlantillas(): Observable<ValorListaOrdenada[]> {
    const url =
      this.DepuraUrl(this.DepuraUrl(this.app.config.apiUrl) + `metadatos/`) +
      "plantillas/";
  ...
  public ObtienePaginaMetadatos(
    plantillaId: string,
    q: ConsultaBackend
  ): Observable<Paginado<DocumentoPlantilla>> {
    const url =
      this.DepuraUrl(this.DepuraUrl(this.app.config.apiUrl) + `metadatos/`) +
      `${plantillaId}/pagina`;
  ...
  public ObtieneDescriptorNodo(tipo: string): Observable<DescriptorNodo> {
    const subject = new AsyncSubject<any>();
    this.ObtieneMetadatos(tipo, "")
      .pipe(first())
      .subscribe(
        (m) => {
          const valor: DescriptorNodo = {
            PropId: null,
            PropNombre: null,
            PropIdraiz: "",
            PropIdPadre: "",
          ...
          if (valor.PropId === null || valor.PropNombre === null) return null;
  ...
  // ---------------------------------------
  // ---------------------------------------
  public SetCachePropiedadContextual(
    propiedad: string,
    origen: string,
    tranid: string,
    valor: any
  ...
  public GetPropiedadCacheContextual(
    propiedad: string,
    origen: string,
    tranid: string
  ): any {
  ...
  public SetCacheInstanciaEntidad(
    tipo: string,
    id: string,
    entidad: any
  ): void {
  ...
  ActualizarEntidad(tipo: string, Id: string, entidad: any): Observable<any> {
    ...
    this.cliente
      ...
      .subscribe(
        (resultado) => {
          this.applog.ExitoT("editor-pika.mensajes.ok-entidad-act", null, {
            nombre: nombre,
          });
  ...
  PostCommand(
    ...
    command: string,
    body: unknown
  ): Observable<RespuestaComandoWeb> {
    const subject = new AsyncSubject<any>();
    this.cliente
      .PostCommand(entidad, command, body)
      .pipe(first())
      .subscribe(
        (r) => {
          subject.next(r);
  ...
  CreaEntidad(tipo: string, entidad: any): Observable<any> {
    ...
    this.cliente
      ...
      .subscribe(
        (resultado) => {
          this.applog.ExitoT("editor-pika.mensajes.ok-entidad-add", null, {
            nombre: nombre,
          });
          subject.next(resultado);
        },
  ...
  CreaEntidadMiembro(
    ...
    const subject = new AsyncSubject<any>();
    this.cliente
      .PostMiembros(idPadre, idMiembros, tipo)
      .pipe(debounceTime(500))
      .subscribe(
        (resultado) => {
          this.applog.ExitoT("editor-pika.mensajes.ok-entidad-add", null, {
  ...
  DeleteTodosVinculados(idPadre: string, entidad: string): Observable<any> {
    const subject = new AsyncSubject<any>();
    this.cliente
      .DeleteTodosVinculados(idPadre, entidad)
      .pipe(debounceTime(500))
      .subscribe(
        (resultado) => {
          this.applog.ExitoT(
            "editor-pika.mensajes.ok-vinculados-del-all",
            null,
            { nombre: "" }
          );
          subject.next(true);
  ...
  public EliminarEntidadMiembros(
    tipo: string,
    idPadre: string,
    idMiembros: string[]
  ): Observable<boolean> {
    const subject = new AsyncSubject<any>();
    this.cliente
      .DeleteMiembros(idPadre, idMiembros, tipo)
      .pipe(debounceTime(500))
      .subscribe(
        (resultado) => {
          this.applog.ExitoT("editor-pika.mensajes.ok-entidad-del", null, {
  ...
  GetFiltroBusqueda(
    entidad: string,
    id: string
  ): Observable<FiltroConsultaPropiedad[]> {
  ...
  SolicitarLista(
    lista: AtributoLista,
    consulta: Consulta
  ): Observable<AtributoLista> {
    let query = "";
    consulta.FiltroConsulta.forEach(
      (x) => (query = query + `${x.Propiedad}-${x.Operador}-${x.Valor}`)
    );
  ...
  public ObtieneMetadatos(
    tipoentidad: string,
    entidadPadre: string = ""
  ): Observable<MetadataInfo> {
    const subject = new AsyncSubject<MetadataInfo>();
    ...
    if (this.cache.has(key)) {
      ...
      this.cliente
        .GetMetadata(tipoentidad)
        .pipe(first())
        .subscribe(
          (m) => {
            this.ProcesaMetadatos(tipoentidad, m)
              .pipe(first())
              .subscribe(
                (procesados) => {
                  if (
                    environment.clouded &&
                    tipoentidad.toLowerCase() == "volumen"
                  ) {
                    procesados.EntidadesVinculadas = [];
                  }
                  this.cache.set(key, procesados);
                  subject.next(
                    this.MetadatosPorEntidad({ ...procesados }, entidadPadre)
                  );
  ...
  private MetadatosPorEntidad(
    metadatos: MetadataInfo,
    entidadPadre: string
  ): MetadataInfo {
    const especificas = [
      ...metadatos.Propiedades.filter(
        (p) =>
          p.Entidad.toLowerCase() == entidadPadre.toLowerCase() &&
          p.Entidad !== ""
      ),
    ];
    const eliminar = [
  ...
  private ProcesaMetadatos(
    entidad: string,
    metadatos: MetadataInfo
  ): Observable<MetadataInfo> {
    const subject = new AsyncSubject<MetadataInfo>();
    this.ts
      .get("entidades.propiedades." + entidad.toLowerCase())
      .pipe(first())
      .subscribe(
        (r) => {
  ...
  // realiza una consulta de pagina relacional
  public ObtenerPaginaRelacional(
    TipoOrigen: string,
    OrigenId: string,
    Entidad: string,
    consulta: Consulta,
    Texto: string = null
  ): Observable<Paginado<any>> {
    const subject = new AsyncSubject<Paginado<any>>();

    this.cliente
      .PageRelated(TipoOrigen, OrigenId, consulta, Entidad, Texto)
      .pipe(debounceTime(500), first())
      .subscribe(
        (data) => {
          this.BuscaTextoDeIdentificadores(Entidad, data)
            .pipe(first())
            .subscribe((isok) => {
              subject.next(data);
              subject.complete();
  ...
  // realiza una consulta de pagina no relacional
  public ObtenerPaginaPersonalizada(
    Entidad: string,
    consulta: unknown,
    url: string
  ): Observable<Paginado<unknown>> {
    const subject = new AsyncSubject<Paginado<any>>();

    this.cliente
      .PostPagePersonalizada(consulta, url)
      .pipe(debounceTime(500), first())
      .subscribe(
        (data) => {
          this.BuscaTextoDeIdentificadores(Entidad, data)
  ...
  public BuscaTextoDeIdentificadores(
    tipoentidad: string,
    pagina: Paginado<any>,
    ...
    if (metadata) {
      ...
      metadata.Propiedades.forEach((p) => {
        ...
        if (p.CatalogoVinculado) {
          this.BuscaIdsParaCatalogos(p, pagina).forEach((item) =>
            buscar.push(item)
          );
        } else {
          if (
            p.AtributoLista &&
            p.AtributoLista.DatosRemotos &&
            p.AtributoLista.Entidad !== ""
          ) {
            this.BuscaIdsParaLista(p, pagina).forEach((item) =>
              buscar.push(item)
            );
          }
        }
          ...
          (results) => {
            ...
            results.forEach((element) => {
              ...
              element.forEach((item: ValorListaOrdenada) => {
                if (
                  this.ListaIds.findIndex(
                    (x) => x.Id === item.Id && x.Entidad === entidad
                  ) < 0
                )
                  this.ListaIds.push(
                    new TextoDesdeId(entidad, item.Id, item.Texto)
                  );
              });
              idx++;
            });
  ...
  public ObtieneCamposCatalogo(metadata: MetadataInfo): Propiedad[] {
    ...
    metadata.CatalogosVinculados.forEach((c) => {
      if (
        metadata.Propiedades.findIndex((x) => x.Id === c.PropiedadReceptora) >=
        0
      ) {
        return;
      }
  ...
  private BuscaIdsParaLista(p: Propiedad, pagina: Paginado<any>): string[] {
    ...
    if (pagina && pagina.Elementos) {
      ...
      pagina.Elementos.forEach((item) => {
        if (
          this.ListaIds.findIndex(
            (x) => x.Id === item[p.Id] && x.Entidad === p.AtributoLista.Entidad
          ) < 0
        ) {
  ...
  public SinopisPorIds(
    consultaId: string,
    pagina: Paginado<any>
  ): Observable<HighlightHit[]> {
  ...
  private MuestraErrorHttp(
    ...
    this.ts
      ...
      .subscribe((t) => {
        ...
        if (
          t["entidades." + modulo] !== "entidades." + modulo &&
        ...
        if (error instanceof HttpResponseBase) {
          switch (error.status) {
            case 400:
              this.applog.FallaT(
                "editor-pika.mensajes.err-datos-erroneos",
                null,
            ...
            case 404:
              this.applog.FallaT(
                "editor-pika.mensajes.err-datos-noexiste",
                null,
                { entidad: trad.singular, prefijo: trad.prefijoSingular }
              );
              break;
            ...
            case 409:
              this.applog.FallaT(
                "editor-pika.mensajes.err-datos-conflicto",
                null,
                { entidad: trad.singular, prefijo: trad.prefijoSingular }
              );
              break;
            ...
            case 500:
              this.applog.FallaT(
                "editor-pika.mensajes.err-datos-servidor",
                null,
                {
                  entidad: trad.singular,
                  prefijo: trad.prefijoSingular,
  ...
  TemaSeleccionAdicionar(
    nombre: string,
    entidad: string
  ): Observable<ValorListaOrdenada[]> {
    const subject = new AsyncSubject<ValorListaOrdenada[]>();
    this.cliente
      .TemaSeleccionAdicionar(nombre, entidad)
      .pipe(first())
      .subscribe(
        (resultado) => {
          const key = this.cache.ClaveSeleccion(entidad);
          ...
          seleccion = seleccion.sort((a, b) =>
            a.Texto < b.Texto ? -1 : a.Texto > b.Texto ? 1 : 0
          );
          this.cache.set(key, seleccion);
          subject.next(seleccion);
  ...
  SeleccionAdicionar(
    ...
    const subject = new AsyncSubject<any>();
    this.cliente
      .SeleccionAdicionar(temaid, Ids, tipo)
      .pipe(first())
      .subscribe(
        (resultado) => {
          this.applog.ExitoT(
            "editor-pika.mensajes.ok-seleccion-add",
            null,
            null
          );
        },
  ...
  TemaEliminar(temaid: string, tipo: string): Observable<any> {
    const subject = new AsyncSubject<boolean>();
    this.cliente
      .TemaSeleccionEliminar(temaid, tipo)
      .pipe(first())
      .subscribe(
        (resultado) => {
          this.applog.ExitoT(
            "editor-pika.mensajes.ok-seleccion-del",
            null,
            null
          );
          subject.next(true);
  ...
  SeleccionEliminar(
    ...
    const subject = new AsyncSubject<boolean>();
    this.cliente
      .SeleccionEliminar(temaid, Ids, tipo)
      .pipe(first())
      .subscribe(
        (resultado) => {
          this.applog.ExitoT(
  ...
  SeleccionVaciar(temaid: string, tipo: string): Observable<any> {
    const subject = new AsyncSubject<boolean>();
    this.cliente
      .SeleccionVaciar(temaid, tipo)
      .pipe(first())
      .subscribe(
        (resultado) => {
          this.applog.ExitoT(
            "editor-pika.mensajes.ok-seleccion-empty",
            null,
            null
          );
          subject.next(true);
