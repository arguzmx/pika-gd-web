...
export class VisorEventosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin([app, usuarios]).subscribe(resultados => {
      ...
      this.T.ObtenerTraduccion(traducciones).pipe(first()).subscribe(p => {
        tmpapp.forEach(a => {
          if (a.Modulos) {
            a.Modulos.forEach(m => {
              if (m.EventosAuditables) {
  ...
  private CargaTraducciones() {
    this.T.ts = [
      'componentes.auditoria.lbl-aplicacion', 'componentes.auditoria.ph-aplicacion',
      'componentes.auditoria.lbl-modulo', 'componentes.auditoria.ph-modulo',
      'componentes.auditoria.lbl-evento', 'componentes.auditoria.ph-evento',
      'componentes.auditoria.lbl-usuarios', 'componentes.auditoria.ph-usuarios',
      'componentes.auditoria.lbl-fechai', 'componentes.auditoria.lbl-fechaf',
      'componentes.auditoria.titulo', "componentes.auditoria.buscar",
      'componentes.auditoria.limpiar', 'ui.sin-resultados-tabla', 
      'auditoria.entidad.Id','auditoria.entidad.Fecha','auditoria.entidad.DireccionRed',
      'auditoria.entidad.IdSesion','auditoria.entidad.UsuarioId','auditoria.entidad.DominioId',
      'auditoria.entidad.UAId','auditoria.entidad.Exitoso','auditoria.entidad.Fuente',
      'auditoria.entidad.AppId','auditoria.entidad.ModuloId','auditoria.entidad.TipoEvento',
      'auditoria.entidad.TipoFalla','auditoria.entidad.TipoEntidad','auditoria.entidad.IdEntidad',
      'auditoria.entidad.NombreEntidad','auditoria.entidad.Delta','auditoria.entidad.TipoEventoTexto',
    ];
  ...
  Buscar() {
    ...
    this.bitacoraService.QueryEventos(q).pipe(first()).subscribe(l => {
      this.ProcesaDatos(l);
    }, (e) => { console.error(e); });
  ...
  private ProcesaDatos(datos: Paginado<EventoAuditoria>) {
    ...
    if(usuariospendientes != '') {
      this.bitacoraService.UsuariosPorIds(usuariospendientes).pipe(first())
      .subscribe(us=> {
    ...
    datos.Elementos.forEach(e=> {
      ...
      if (app) {
        ...
        if (mod) {
          ...
          const tipo = `${app.Id}$${mod.Id.replace( `${mod.AplicacionId}-`, '')}$${e.TipoEntidad}$${e.TipoEvento}`;
